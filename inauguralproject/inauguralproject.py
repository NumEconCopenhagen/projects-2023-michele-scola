...
class HouseholdSpecializationModelClass:
    ...
    def calc_utility(self,LM,HM,LF,HF):
        ...
        if par.sigma == 0:
            ...
            H = (HM+1e-8)**(1-par.alpha+1e-8)*(HF+1e-8)**par.alpha+1e-8
        else:
            H = ((1-par.alpha+1e-8)*(HM+1e-8)**((par.sigma-1)/(par.sigma+1e-8))+(par.alpha+1e-8)*(HF+1e-8)**((par.sigma-1)/(par.sigma+1e-8)))**((par.sigma+1e-8)/(par.sigma-1+1e-8))
    ...
    def solve(self, do_print=False):
        ...
        #call solver
        solution = optimize.minimize(obj, x_guess, method='SLSQP', bounds=bounds, constraints=constraints)
